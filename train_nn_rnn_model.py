# 该文件是彻底的失败，原本初次使用class_weights就是最好的结果，后来尝试了LSTM反而越来越糟，且难以找出问题。
# 出于困惑和保险，我复制了一份old_train_nn_rnn_model.py。该版本f1-score最高，约0.5，而且还快。但是成绩不稳定（没有使用SEED）
...
from tensorflow.keras.layers import Embedding, GlobalAveragePooling1D, Dense, LSTM, Dropout # 我们要用的三种"乐高积木"。---VERSION 3：注意这里新增了LSTM+Dropout ---
...
# ID 0  |  0.00    0.00    0.00  ...    0.00   |
#       +--------------------------------------+
model.add(Embedding(input_dim=10000, output_dim=64, input_length=maxlen))   # --- VERSION 3: 16改成了64 ---
...
# --- VERSION 3核心升级：用LSTM替换GlobalAveragePooling1D ---
# 第二层：LSTM层 - 我们的“语言学专家”
# 64是LSTM单元的数量，可以理解为这位专家的“脑容量”或“记忆力”
model.add(LSTM(64))

# 加入“镇定剂”！Dropout层
# 0.5意味着在每次训练迭代中，随机关闭50%的神经元
...
class_weights = {0: np.float64(1.0), 1: np.float64(7.625)}
...
history = model.fit(X_train_padded, y_train,
                    ...
                    #class_weight=class_weights,  # <-- VERSION2：加入了赏罚机制     <-- VERSION3：去掉了
                    verbose=2)
print("模型训练完成。")
